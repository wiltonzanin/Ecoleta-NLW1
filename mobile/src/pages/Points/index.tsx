...
const Points = () => {
  ...
  useEffect(() => {
    async function loadPosition() {
      ...
      if (status !== 'granted') {
        Alert.alert('Aviso!', 'Precisamos da sua permição para obter a localização.');
        return;
      }
    ...
    <>
      <View style={styles.container}>
        ...
        <View style={styles.mapContainer}>
          {initialPosition[0] !== 0 && (
            <MapView style={styles.map} initialRegion={{ latitude: initialPosition[0], longitude: initialPosition[1], latitudeDelta: 0.100, longitudeDelta: 0.100, }}>
              {points.map(point => (
                <Marker style={styles.mapMarker} key={String(point.id)} onPress={() => handleNavigateToDetail(point.id)} coordinate={{ latitude: point.latitude, longitude: point.longitude }}>
